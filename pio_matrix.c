...
int main()
{
    ...
    gpio_init(botãoA);
    gpio_set_dir(botãoA, GPIO_IN);
    gpio_pull_up(botãoA);

    //inicializar o botão de interrupção - GPIO6
    gpio_init(botãoB);
    gpio_set_dir(botãoB, GPIO_IN);
    gpio_pull_up(botãoB);
    ...
    gpio_set_irq_enabled_with_callback(botãoA, GPIO_IRQ_EDGE_FALL, 1, & gpio_irq_handler);
    gpio_set_irq_enabled(botãoB, GPIO_IRQ_EDGE_FALL, true);
